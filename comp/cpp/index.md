...
試しにコードを実行してみましょう。
好きな場所をVSCodeで開いてください(WSLの場合WSL上で`code .`です)。
...
$ 0 \leq N \leq 30 \\ Nは整数である $

なるほど。色々解法はありますが、Nが30以下と小さいので、最もわかりやすい「2を真面目にN回掛ける」という方法で解いてみます。

このように書くことができます。入出力の部分が他の言語とかなり違うので、よく見てください。
...
実際にコンパイル→実行してみましょう。実行した途端に処理が止まるような感じになりますが、これは入力待ちです。
