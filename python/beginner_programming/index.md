...
#### 問題2
以下のように3行の文章を出力してください。(ファイルは先ほどのhello.pyを編集して使ってもいいし、新しくbeginner_programmingディレクトリの中に作ってそこに書いてもいいです。)
...
この辺の操作はプログラミングと直接は関係ありませんがきちんとできるようにしておきましょう。また、もちろんこれらの操作はプロジェクトを作成する時にLocationで指定した場所にエクスプローラーで行けばエクスプローラー内で普通にできます。基本はPyCharm上で操作すれば良いのですが、この課題をやってるうちに変なことになっちゃったからエクスプローラーで一回確認したいとかだったらどうぞ。そうでなくても一度はエクスプローラーの中で今作業してるpythonファイルはこれなのかあって感じで一度は確認してみてください。
...
もう使ってる人もいるかもしれませんが、コピー、ペースト、カットを使えば先ほどのコードを使って楽にコーディングできます。先ほどコピペはやめろと言いましたがあれはお手本のコードを脳死でコピペして実行がうまくいってそれで満足して次に進むな、ということです。コードはどんどん再利用してください。
...
### 内容補足2
この章では変数を使うということでいちいち数字を変数に代入してから計算していますが、あくまでも代入なので以下の4種類どの書き方でも同じです。それではどう使い分けるかというと、のちにそのaとかbとかの変数を再利用するかどうかです。
``` python
a = 2
b = 3
c = a + b
...
print(a)
```
答えは`4`です。2行目に違和感を感じましたか？それは「`=`は代入」という考えにまだなれていません。`=`は「両辺が等しい」ことを示すものではありません。後に勉強しますがこれには`==`を使います。というわけで`a = a + 3`というのは`a`に3を足して更新する、という意味です。とはいえこの書き方を直接することはほとんどなく以下のように簡略化して書きます。
``` python
a += 3
```
他の演算子についても同様です。
...
print(base ** (1 / exp))
```
またprint文の中の計算式のように演算の計算順序の決定のために`()`を使えます。ただし数学との大きな違いは`()`の外側は`{}`や`[]`は使うことなく`()`でその上から囲み続けることです。
